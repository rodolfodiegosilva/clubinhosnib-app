...
import { validateMediaURL } from "utils/validateMediaURL"; // ✅ Importa função centralizada

interface Props {
  documents: StudyMediaItem[];
  setDocuments: (docs: StudyMediaItem[]) => void;
}

export default function StudyDocuments({ documents, setDocuments }: Props) {
  const [newDoc, setNewDoc] = useState<StudyMediaItem>({
    title: "",
    description: "",
    type: "link",
