...
const hasRoutePath = (page: IdeasPageData | null): page is IdeasPageData & { route: { path: string } } => {
...
export default function IdeasPageDetailsModal({ page, open, onClose }: Props) {
          ...
          {items.map((media) => (
            <Grid item xs={12} sm={6} md={4} key={media.id}>
              ...
              >
                ...
                <Typography mt={1}><strong>Título:</strong> {media.title}</Typography>
                <Typography variant="body2"><strong>Descrição:</strong> {media.description}</Typography>
                <Typography variant="body2"><strong>Upload:</strong> {media.uploadType}</Typography>
                <Typography variant="body2"><strong>Local:</strong> {media.isLocalFile ? "Sim" : "Não"}</Typography>
                {media.originalName && (
                  <Typography variant="body2"><strong>Arquivo:</strong> {media.originalName}</Typography>
                )}
                {media.size && (
                ...
                <Box display="flex" alignItems="center" gap={1} mt={1} flexWrap="wrap">
                  <Typography variant="body2" sx={{ wordBreak: "break-word", flex: 1 }}>
                    <strong>URL:</strong> <a href={media.url} target="_blank" rel="noopener noreferrer">{media.url}</a>
                  </Typography>
                  <IconButton
                    size="small"
  ...
  return (
    ...
    >
      ...
      <DialogTitle id="dialog-title" sx={{ textAlign: "center", fontWeight: "bold", fontSize: "1.5rem" }}>
        ...
        {page && (
          <Stack spacing={4}>
            ...
            <Paper elevation={2} sx={{ p: 3, borderRadius: 2 }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}><Typography><strong>Título:</strong> {page.title}</Typography></Grid>
                <Grid item xs={12} md={6}><Typography><strong>Subtítulo:</strong> {page.subtitle}</Typography></Grid>
                <Grid item xs={12}><Typography sx={{ whiteSpace: "pre-wrap" }}><strong>Descrição:</strong> {page.description}</Typography></Grid>
                <Grid item xs={12} md={6}><Typography><strong>Criado em:</strong> {formatDate(page.createdAt)}</Typography></Grid>
                <Grid item xs={12} md={6}><Typography><strong>Atualizado em:</strong> {formatDate(page.updatedAt)}</Typography></Grid>
                <Grid item xs={12}>
                  <Chip
            ...
            {Array.isArray(page.sections) && page.sections.map((section) => (
              <Paper key={section.id} elevation={2} sx={{ p: 3, borderRadius: 2 }}>
                <Typography variant="h6" textAlign="center" fontWeight="bold" color="primary" mb={2}>
                  {section.title}
                </Typography>
                <Typography mb={1}><strong>Descrição:</strong> {section.description}</Typography>
                <Chip
